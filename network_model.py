...
# Function to create the graph with nodes and edges
def create_network_graph():
    G = nx.Graph()
    
    for server, location in servers.items():
        G.add_node(server, pos=(location[1], location[0]))  # longitude, latitude for plotting
    
    # Add edges based on a distance threshold
    THRESHOLD = 2000  # You can adjust this threshold if needed
    for server1, loc1 in servers.items():
        for server2, loc2 in servers.items():
            if server1 != server2:
                distance = geodesic(loc1, loc2).kilometers
                if distance <= THRESHOLD:
                    G.add_edge(server1, server2, weight=distance)
